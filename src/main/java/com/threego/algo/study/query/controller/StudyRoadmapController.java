...
import com.threego.algo.study.query.service.StudyRoadmapService;
...
@Tag(name = "Study Roadmap", description = "스터디 로드맵 조회 API")
@RestController
...
public class StudyRoadmapController {
    ...
    public ResponseEntity<List<StudyRoadmapDTO>> findRoadmaps(@PathVariable Integer studyId) {
        List<StudyRoadmapDTO> roadmaps = studyRoadmapService.findAllStudyRoadmap(studyId);
    ...
    public ResponseEntity<StudyRoadmapDetailDTO> findRoadmapDetail(@PathVariable Integer roadmapId) {
        StudyRoadmapDetailDTO roadmapDetail = studyRoadmapService.findStudyRoadmapDetail(roadmapId);
